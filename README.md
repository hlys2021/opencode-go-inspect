...
* **访问 Web 看板**：打开浏览器访问 [http://127.0.0.1:8000](http://127.0.0.1:8000) 即可查看可视化图表与详细面板。
...
3. **预算告警 (Budget Warning)**：
   * 自动监控当月消耗。当达到预算 80% 时触发**预警（Warning）**；达到或超过 100% 时触发**严重告警（Critical）**，并在 Web 看板顶端以显著颜色预警。
4. **一键 CSV 数据导出**：
